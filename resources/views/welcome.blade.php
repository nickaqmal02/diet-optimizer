...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Laravel') }} - Diet Optimizer</title>
    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col">
        ...
        <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0">
            <main class="flex max-w-[335px] w-full flex-col lg:max-w-5xl lg:flex-row gap-4">
                
                <!-- LEFT PANEL: BMR Calculator + Goal + Budget -->
                <div class="bg-white dark:bg-[#161615] rounded-lg shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] p-6 flex-1">
                    <h2 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">📊 Step 1: Your Body Profile</h2>
                    
                    <div class="grid grid-cols-2 gap-3 mb-4">
                        <div>
                            <label class="text-sm text-gray-600 dark:text-gray-400">Gender</label>
                            <select id="gender" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                                <option value="male">Male</option>
                                <option value="female">Female</option>
                            </select>
                        </div>
                        <div>
                            <label class="text-sm text-gray-600 dark:text-gray-400">Age (years)</label>
                            <input type="number" id="age" value="22" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                        </div>
                        <div>
                            <label class="text-sm text-gray-600 dark:text-gray-400">Weight (kg)</label>
                            <input type="number" id="weight" value="65" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                        </div>
                        <div>
                            <label class="text-sm text-gray-600 dark:text-gray-400">Height (cm)</label>
                            <input type="number" id="height" value="170" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                        </div>
                        <div class="col-span-2">
                            <label class="text-sm text-gray-600 dark:text-gray-400">Activity Level</label>
                            <select id="activity" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                        ...
                        <div class="col-span-2">
                            <label class="text-sm text-gray-600 dark:text-gray-400">🍽️ Meal Size (% of daily calories)</label>
                            <input type="range" id="mealPortion" min="0.2" max="0.5" step="0.01" value="0.33" class="w-full" oninput="updateMealPortionLabel(this.value)">
                            <div class="flex justify-between text-xs mt-1">
                                <span class="text-gray-600 dark:text-gray-400">Small (20%)</span>
                                <span id="mealPortionLabel" class="text-gray-900 dark:text-white font-medium">33%</span>
                                <span class="text-gray-600 dark:text-gray-400">Large (50%)</span>
                            </div>
                        </div>
                    </div>

                    <button onclick="calculateBMR()" class="w-full bg-gray-900 dark:bg-gray-100 hover:bg-gray-700 dark:hover:bg-gray-300 text-white dark:text-gray-900 py-2 rounded-md font-medium mb-4 transition">
                        Calculate My TDEE →
                    </button>

                    <div id="bmrResult" class="bg-gray-100 dark:bg-[#0a0a0a] p-4 rounded-md mb-4 hidden">
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Your maintenance calories: <span id="tdeeValue" class="text-lg font-bold text-[#f53003]">0</span> kcal/day</p>
                        <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">↓ Adjust below based on your goal ↓</p>
                    </div>

                    <h2 class="text-lg font-semibold mt-4 mb-2 text-gray-900 dark:text-white">🎯 Step 2: Your Goal & Budget</h2>
                    
                    <div class="mb-3">
                        <label class="text-sm text-gray-600 dark:text-gray-400">Health Goal</label>
                        <select id="goal" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                            <option value="deficit">🔥 Weight Loss (Deficit: -400 kcal)</option>
                            <option value="maintain" selected>⚖️ Maintain Weight (TDEE)</option>
                            <option value="surplus">💪 Muscle Gain (Surplus: +400 kcal)</option>
                        </select>
                    </div>

                    <div class="mb-4">
                        <label class="text-sm text-gray-600 dark:text-gray-400">Your Budget (RM)</label>
                        <input type="number" id="budget" value="10" class="w-full border border-gray-300 dark:border-[#3E3E3A] rounded-md px-3 py-2 bg-white dark:bg-[#161615] text-gray-900 dark:text-white">
                    </div>

                    <button onclick="optimizeMeal()" class="w-full bg-[#f53003] hover:bg-[#d42a02] text-white py-2 rounded-md font-medium transition">
                        🍽️ Find Optimal Meal (LP Solver)
                    </button>
                </div>

                <!-- RIGHT PANEL: Results -->
                <div class="bg-white dark:bg-[#161615] rounded-lg shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] p-6 flex-1">
                    <h2 class="text-lg font-semibold mb-3 text-gray-900 dark:text-white">✅ Your Optimal Meal</h2>
                    <div id="resultArea" class="text-sm text-gray-600 dark:text-gray-400">
                        <p class="text-center py-8">Enter your stats and click "Find Optimal Meal"</p>
                    </div>
                    
                    <div class="mt-6 pt-4 border-t border-gray-200 dark:border-[#3E3E3A]">
                        <p class="text-xs text-center text-gray-500 dark:text-gray-500">
                            💡 <span class="font-medium">Guest Mode</span> — 
                            <a href="{{ route('register') }}" class="text-[#f53003] underline">Login/Register</a> to save your profile & daily budget
                        </p>
                    </div>
                </div>
            </main>
...
<script>
    // ---------- FOOD DATABASE ----------
    const FOODS = [
        { name: "Sardines (can)", price: 4.50, calories: 220, protein: 25 },
        { name: "Fried Chicken (pc)", price: 4.00, calories: 250, protein: 20 },
        { name: "Fried Egg", price: 1.80, calories: 90, protein: 7 },
        { name: "Tauhu (2 pcs)", price: 2.00, calories: 120, protein: 10 },
        { name: "White Rice (portion)", price: 1.50, calories: 200, protein: 4 },
        { name: "Mixed Veggies", price: 2.00, calories: 80, protein: 3 },
        { name: "Tempeh (100g)", price: 1.50, calories: 150, protein: 9 },
        { name: "Hard Boiled Egg", price: 1.50, calories: 78, protein: 6 },
        { name: "Tuna Sandwich", price: 5.00, calories: 320, protein: 18 },
        { name: "Banana", price: 1.00, calories: 105, protein: 1 },
        { name: "Protein Shake (scoop)", price: 6.00, calories: 120, protein: 24 },
        { name: "Nasi Lemak (small)", price: 3.50, calories: 400, protein: 8 },
        { name: "Maggi Goreng", price: 4.50, calories: 480, protein: 10 },
        { name: "Roti Canai (1pc)", price: 1.50, calories: 200, protein: 5 },
        { name: "Dhal Curry (side)", price: 1.00, calories: 80, protein: 3 }
    ];

    let currentTDEE = 0;
    let calorieTargetPerMeal = 0;
    ...
    function calculateBMR() {
        ...
        document.getElementById('tdeeValue').innerText = currentTDEE;
    ...
    function updateCalorieTarget() {
        ...
        if (goal === 'deficit') {
            dailyTarget = Math.max(1400, currentTDEE - 400);
        } else if (goal === 'surplus') {
            dailyTarget = currentTDEE + 400;
        } else {
            dailyTarget = currentTDEE;
        }
        
        const mealPortion = parseFloat(document.getElementById('mealPortion')?.value || '0.33');
        calorieTargetPerMeal = Math.round(dailyTarget * mealPortion);
        
        return calorieTargetPerMeal;
    }
    // where we update the calorie target
    function updateMealPortionLabel(val) {
        document.getElementById('mealPortionLabel').innerText = Math.round(val * 100) + '%';
        updateCalorieTarget();
    }

    // LP Solver (Brute Force)
    function optimizeMeal() {
        if (currentTDEE === 0) {
            alert("Please click 'Calculate My TDEE' first!");
            return;
        }
        ...
        let maxCalories = calorieTargetPerMeal * 1.2;
        let minCalories = calorieTargetPerMeal * 0.8;
        
        minCalories = Math.max(minCalories, 300);
        maxCalories = Math.min(maxCalories, 1200);

        let bestSolution = null;
        let bestProtein = -1;

        function search(index, totalPrice, totalCal, totalProtein, solution) {
            if (index === FOODS.length) {
                // HERE IS OUR CONSTRAINS MUST BE IN RANGE BUDGET AND ALSO RANGE OF CALORIES
                if (totalPrice <= budget && totalCal >= minCalories && totalCal <= maxCalories) {
                    if (totalProtein > bestProtein) {
                        // OUR OBJECTIVE IS FINDING THE BEST PROTEIN
            ...
            let maxQty = Math.min(4, Math.floor(budget / food.price));
            
            for (let qty = 0; qty <= maxQty; qty++) {
                if (totalPrice + (food.price * qty) <= budget) {
                    solution.push({ ...food, qty });
                    search(index + 1, totalPrice + (food.price * qty), totalCal + (food.calories * qty), totalProtein + (food.protein * qty), solution);
                    solution.pop();
                }
            }
        }
        ...
        if (!bestSolution || bestSolution.items.length === 0 || bestSolution.items.every(i => i.qty === 0)) {
            resultDiv.innerHTML = `
                <div class="bg-red-50 dark:bg-red-950/20 p-4 rounded-md text-center">
                    <p class="text-[#f53003] font-medium">⚠️ No feasible meal found</p>
                    <p class="text-sm mt-2 text-gray-700 dark:text-gray-300">Suggestions:</p>
                    <ul class="text-xs mt-1 text-left list-disc list-inside text-gray-600 dark:text-gray-400">
                        <li>Increase budget (currently RM${budget})</li>
                        <li>Adjust "Meal Size" to a smaller portion</li>
                        <li>Try a different goal (deficit = smaller meals)</li>
                    </ul>
                    <p class="text-xs mt-3 text-gray-700 dark:text-gray-300">🎯 Your target: ${calorieTargetPerMeal} kcal per meal</p>
                    <p class="text-xs mt-1 text-gray-600 dark:text-gray-400">📊 Daily TDEE: ${currentTDEE} kcal</p>
                </div>
            `;
            return;
        }
        ...
        let itemsHtml = '';
        bestSolution.items.forEach(item => {
            if (item.qty > 0) {
                itemsHtml += `
                    <div class="flex justify-between items-center py-2 border-b border-gray-200 dark:border-[#3E3E3A]">
                        <div>
                            <span class="font-medium text-gray-900 dark:text-white">${item.name}</span>
                            <span class="text-xs text-gray-500 dark:text-gray-400 ml-2">x${item.qty}</span>
                        </div>
                        <div class="text-right">
                            <div class="text-gray-900 dark:text-white">RM ${(item.price * item.qty).toFixed(2)}</div>
                            <div class="text-xs text-gray-500 dark:text-gray-400">${item.calories * item.qty} cal | ${item.protein * item.qty}g protein</div>
                        </div>
                    </div>
                `;
            }
        ...
        const goalText = document.getElementById('goal').value === 'deficit' ? '🔥 Weight Loss' : 
                         (document.getElementById('goal').value === 'surplus' ? '💪 Muscle Gain' : '⚖️ Maintenance');
        
        const isWithinTarget = bestSolution.totalCal >= minCalories && bestSolution.totalCal <= maxCalories;
        
        resultDiv.innerHTML = `
            <div class="bg-green-50 dark:bg-green-950/20 p-3 rounded-md mb-4">
                <p class="text-sm font-medium text-gray-900 dark:text-white">🎯 Per meal target: ${calorieTargetPerMeal} kcal (${goalText})</p>
                <p class="text-xs text-gray-600 dark:text-gray-400">Daily TDEE: ${currentTDEE} kcal | Budget: RM${budget} for this meal</p>
            </div>
            <div class="space-y-1 max-h-80 overflow-y-auto">
                ${itemsHtml}
            </div>
            <div class="mt-4 pt-3 border-t-2 border-[#f53003]">
                <div class="flex justify-between font-bold text-gray-900 dark:text-white">
                    <span>TOTAL</span>
                    <span>RM ${bestSolution.totalPrice} | ${bestSolution.totalCal} cal | ${bestSolution.totalProtein}g protein</span>
                </div>
                ${isWithinTarget ? 
                    '<p class="text-green-600 dark:text-green-400 text-xs mt-2">✅ Perfect! This meal fits your goal perfectly.</p>' : 
                    `<p class="text-amber-600 dark:text-amber-500 text-xs mt-2">⚠️ Calories: ${bestSolution.totalCal} vs target: ${calorieTargetPerMeal}</p>`
                }
            </div>
        `;
    }
